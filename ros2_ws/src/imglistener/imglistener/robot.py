...
class Robot():
    ...
    def update_robot(self, kp_clean, des_clean, depth_frame, frame_index, frame_counter, kinect_to_base_matrix, base_to_kinect_matrix):
        ...
        visible_des, visible_pts_glob_2d, visible_map_indices = self.algorithmen.test_only_for_visible_landmarks(self.map_manager.landmarks, self.curr_pos_x, self.curr_pos_y, self.curr_theta, base_to_kinect_matrix)
            ...
            if len(visible_des) > 0:
                ...
                if len(matches) > self.config.min_matches:
                    ...
                    if delta_R is not None:
                        ...
                        epsilon_x = np.random.normal(0, sigma_x**2)
                        epsilon_y = np.random.normal(0, sigma_y**2)
                        epsilon_theta = np.random.normal(0, sigma_theta**2)


                        # update current pose with the estimated transformation
                        self.curr_pos_x += delta_tx_odom + epsilon_x
                        self.curr_pos_y += delta_ty_odom + epsilon_y
                        self.curr_theta += delta_theta + epsilon_theta
                        pose_updated = True
                        ...
                        self.map_manager.add_new_landmarks(
                            ...
                            (self.curr_pos_x, self.curr_pos_y, self.curr_theta), frame_index
                        )
        ...
        return pose_updated, self.curr_pos_x, self.curr_pos_y, self.curr_theta, log_robot_likelihood