...
class Robot():
    ...
    def update_robot(self, kp_clean, des_clean, depth_frame, frame_index, frame_counter, base_to_kinect_matrix, local_robot_pts_3d):
        ...
        if frame_counter <= 0:
                ...
                if len(matches) > self.config.min_matches:
                    ...
                    if delta_R is not None:
                        if (np.linalg.norm(delta_t) > self.config.ransac_max_deviation_delta or
                            ...
                            print(f"RANSAC verworfen: |Δt|={np.linalg.norm(delta_t):.1f}mm, "
                                f"Δθ={np.degrees(delta_theta):.1f}°")
                            delta_R = None
                        # → fällt in den Fehlerast
            ...
            print(f"Skipping frame {frame_index} to increase stability. Frame counter: {frame_counter}")
            log_robot_likelihood = self.config.partical_filter_fail_standart_error # very low likelihood if we are skipping frames to discourage pose updates during these frames
