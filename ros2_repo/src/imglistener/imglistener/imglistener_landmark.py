...
class ImageSubscriber(Node):
    def __init__(self):
        ...
        # Focal length 
        self.f = 526.61
        self.width = 2 * self.cu
        self.fov = 2 * math.atan2(self.width / (2 * self.f)) # calculate field of view based on image width and focal length
...
# odom publisher und in RVIZ2 anschauen
    ...
    def get_landmarks_in_cone(self):
        in_view_indeces = []
        in_view_des = []
        for i, lm in enumerate(self.map_landmarks):
            # Berechnung des Winkels zur Landmarke
            dx = lm['world_coordinates'][0] - self.curr_pos_x
            dy = lm['world_coordinates'][1] - self.curr_pos_y
            dist = math.sqrt(dx**2 + dy**2)

            # Ueberpruefe Distanz
            if dist > self.max_dist or dist < self.min_dist:
                continue # Überspringe Landmarken außerhalb des Distanzbereichs

            #Ueberpruefe Winkel
            # Normalisieren des Winkels auf [-pi, pi]
            angle_to_lm = math.atan2(dy, dx)
            relevant_angle = (angle_to_lm - self.curr_theta + math.pi) % (2 * math.pi) - math.pi

            if abs(relevant_angle) < self.fov / 2:
                in_view_indeces.append(i)
                in_view_des.append(lm['des'])

        return in_view_indeces, in_view_des
    ...
    def listener_callback_rgb(self,msg):
        ...
        idx_in_cone, des_in_cone = self.get_landmarks_in_cone()

        if(kp_clean is None or des_clean is None):
            return # skip processing if no valid keypoints/descriptors are found

        #Lokale 3D-Koordinaten der Keypoints berechnen und in Liste speichern
        ...
        if(self.frame_counter == 0):
            if(self.des_queue is not None):
                matches = self.bf.match(self.des_queue, des_clean)
